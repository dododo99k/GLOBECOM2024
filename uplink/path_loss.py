...
def path_loss_calculator(frequency, distance, ant_height, ant_type,
    ...
    if 0.05 < frequency <= 100:
        ...
        # if indoor, then there will be penetration loss (additional shadowing)
        ...
        if settlement_type == 'suburban' or settlement_type == 'rural':
            ...
            if 10 <= d2d <= dbp:
                if type_of_sight == 'los':
                    return pl1 + np.random.normal(0, 4) # shadowing for pl1
            pl_rma_los = pl1
            ...
            if dbp <= d2d <= 10000:
                if type_of_sight == 'los':
                    return pl2 + np.random.normal(0, 6) # shadowing for pl2
            pl_rma_los = pl2
            ...
            if type_of_sight == 'nlos':
                ...
                return pl_rma_nlos + np.random.normal(0, 8) # shadowing for nlos

            if d2d > 10000:
                return uma_nlos_optional(frequency, distance, ant_height, ue_height,
                    seed_value, iterations) + np.random.normal(0, 8) # shadowing for nlos
            ...
            if d_apost_bp <= d2d <= 5000:
                if type_of_sight == 'los':
                    return pl2 + np.random.normal(0, 4) # shadowing for pl1
            pl_uma_los = pl2
            ...
            if type_of_sight == 'nlos':
                ...
                return pl_uma_nlos + np.random.normal(0, 7.8) # shadowing for pl1
            ...
            if d_apost_bp <= d2d <= 5000:
                if type_of_sight == 'los':
                    return pl2 + np.random.normal(0, 4) # shadowing for pl2
            pl_umi_los = pl2
            ...
            if type_of_sight == 'nlos':
                ...
                if d2d > 5000:
                    pl_apostrophe_umi_nlos = umi_nlos_optional(frequency, distance, ant_height,
                        ue_height, seed_value, iterations)

                pl_uma_nlos = max(pl_apostrophe_umi_nlos, pl_umi_los)

                return pl_uma_nlos + np.random.normal(0, 8.2) # shadowing for pl1
...
def umi_nlos_optional(frequency, distance, ant_height, ue_height,
    seed_value, iterations):
    """
    ...
    path_loss = 32.4 + 20*np.log10(fc) + 31.9*np.log10(d3d)

    random_variation = generate_log_normal_dist_value(
        frequency, 1, 8.2, iterations, seed_value
    )
